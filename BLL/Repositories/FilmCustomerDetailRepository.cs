...
namespace BLL.Repositories
{
    public class FilmCustomerDetailRepository : IFilmBox<FilmCustomerDetailCs>
    {
        ...
        {
            FilmCustomerDetail gelen= db.FilmCustomerDetails.Find(id);
            db.FilmCustomerDetails.Remove(gelen);
            db.SaveChanges();
        }

        public List<FilmCustomerDetailCs> GetAll()
        {
            return db.FilmCustomerDetails.Select(f=>new FilmCustomerDetailCs { FilmCustomerDetailID=f.FilmCustomerDetailID,FilmId=f.FilmId ,CustomerId=f.CustomerId } ).ToList();
        }
        ...
        {
            db.FilmCustomerDetails.Add(new FilmCustomerDetail { FilmId=item.FilmId,CustomerId=item.CustomerId}).ToString();
            db.SaveChanges();
        }

        public void Update(FilmCustomerDetailCs item)
        {
            FilmCustomerDetail gelen = db.FilmCustomerDetails.Find(item.FilmCustomerDetailID);
