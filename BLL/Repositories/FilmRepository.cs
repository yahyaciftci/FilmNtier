...
namespace BLL.Repositories
{
    public class FilmRepository : IFilmBox<FilmCs>
    {
        ...
        {
            Film gelen = db.Films.Find(id);
            db.Films.Remove(gelen);
            db.SaveChanges();
        }
        ...
        {
            db.Films.Add(new Film { FilmName = item.FilmName, Views = item.Views, Vote = (int)item.Vote, FilmDuration = item.FilmDuration, DateOfConstruction = (DateTime)item.DateOfConstruction, Language = item.Language, FilmEndorsement = (decimal)item.FilmEndorsement, Status = item.Status });
            db.SaveChanges();
        }

        public void Update(FilmCs item)
        {
            Film gelen = db.Films.Find(item.FilmId);
